...
const Article = ({article}) => {
    ...
    return (
        <div id='article-container'>
          <div id='article-title'> <Link style={{'textDecoration': 'none'}} key={article._id} to={`/articles/${article._id}`} >  <p id='article-title-wording' >{article.title}</p> </Link></div>
          <div id='user'><User name={article.created_by.username} avatar={article.created_by.avatar_url} /></div>
            ...
            <p id='article'><Link style={{'textDecoration': 'none'}} key={article._id} to={`/articles/${article._id}`} >{article.body}</Link></p>
